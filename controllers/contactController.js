...
exports.getContactById = asyncHandler(async (req, res) => {
  ...
  if (contact.user_id.toString() !== req.user.id) {
    res.status(403);
    throw new Error(
      "User does not have permission to get other user's contacts!"
    );
  }
...
exports.updateContactById = asyncHandler(async (req, res) => {
  ...
  if (contact.user_id.toString() !== req.user.id) {
    res.status(403);
    throw new Error(
      "User does not have permission to update other user's contacts!"
    );
  }
...
exports.deleteContactById = asyncHandler(async (req, res) => {
  ...
  if (contact.user_id.toString() !== req.user.id) {
    res.status(403);
    throw new Error(
      "User does not have permission to delete other user's contacts!"
    );
  }
